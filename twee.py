...
def calculate_score(matched_links, linked_entities, matched_nouns, matched_adj, matched_verbs, important_information):
    score = 100
    print("matched_links", matched_links, linked_entities)
    if(len(matched_links) < len(linked_entities)):
        score -= int((len(linked_entities) - len(matched_links)) / len(linked_entities))*100
    elif(len(linked_entities) == 0):
        unique_to_list = list(set(entities) - set(entities2))
        if(len(unique_to_list) > 0):
            score -= (len(unique_to_list)/len(entities)) * 100
            print("Reason: Important Entities missing ", unique_to_list)


    if(len(matched_nouns) < len(important_information["NOUN"])):
        score -= int((len(important_information["NOUN"]) - len(matched_nouns)) / len(important_information["NOUN"]))*100
    ...
    if(len(matched_adj) < len(important_information["ADJ"])):
        score -= int((len(important_information["ADJ"]) - len(matched_adj)) / len(important_information["ADJ"]))*100
    ...
    if(len(matched_verbs) < len(important_information["VERB"])):
        score -= int((len(important_information["VERB"]) - len(matched_verbs)) / len(important_information["VERB"]))*100
...
score = calculate_score(matched_links, linked_entities, matched_nouns, matched_adj, matched_verbs, important_information)
print("score", score)
...
# wiki_search_results = search_wikipedia(query)
...
#     temp_split = text.split("\n")
    ...
    # score_arr = []
    # for wiki_para in temp_split:
    #     print(wiki_para)
    #     print("\n\n")

    #     # Extract important keywords and entities from the question
    #     entities3 = extract_entities(wiki_para, 1)
    #     preprocessed_text3 = preprocess_text(wiki_para, 1)
    #     important_keywords3, important_information3 = find_important_keywords(preprocessed_text3, entities3)
    #     linked_entities3 = entity_linking(preprocessed_text3, entities3)


    #     matched_links3 = check_entity_links(linked_entities, linked_entities3)
    #     # print("matched_links", matched_links)
    #     # print("Check", len(matched_links), len(linked_entities))

    #     matched_nouns3, matched_adj3, matched_verbs3 = check_keyword_matches(important_information, important_information3)
    #     # print("Check Noun", len(matched_nouns), len(important_information["NOUN"]))
    #     # print("Check ADJ", len(matched_adj), len(important_information["ADJ"]))
    #     # print("Check Verb", len(matched_verbs), len(important_information["VERB"]))

    #     score = calculate_score(matched_links3, linked_entities, matched_nouns3, matched_adj3, matched_verbs3, important_information)
...
# text = get_wikipedia_text("https://en.wikipedia.org/wiki/Louvre")
# temp_split = text.split("\n")
# # print("temp_split", temp_split)
...
#     print("preprocessed_text3", preprocessed_text3)
